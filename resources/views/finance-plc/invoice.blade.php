...
<head>
    ...
    <style>
        ...
        @page {
            size: A4;
            margin: 0;
            /* We'll handle margins inside the container */
        }
        ...
        <div class="content">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6 text-sm">
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        <div class="flex"><span
                                class="font-semibold w-28">Name:</span><span>{{ $order->buyer_name }}</span></div>
                        <div class="flex"><span
                                class="font-semibold w-28">ID:</span><span>{{ $order->buyer_id }}</span></div>
                        <div class="flex"><span
                                class="font-semibold w-28">Address:</span><span>{{ $order->buyer_address }}</span></div>
                        <div class="flex"><span class="font-semibold w-28">Phone:</span><span>{{ $order->phone_1 }}
                                <br>{{ $order->phone_2 }}</span></div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        <div class="flex"><span
                                class="font-semibold w-28">Item:</span><span>{{ $order->item_name }}</span></div>
                        <div class="flex"><span
                                class="font-semibold w-28">Colour:</span><span>{{ $order->colour }}</span></div>
                        <div class="flex"><span
                                class="font-semibold w-28">EMI:</span><span>{{ $order->emi_number }}</span></div>
                        <div class="flex"><span class="font-semibold w-28">Price:</span><span>LKR
            ...
            <div class="text-right text-gray-700 font-medium mt-4 mb-4">
                ...
                <span class="text-blue-600 font-semibold">LKR {{ number_format($min_payment, 2) }}</span>
            </div>
                ...
                <tbody>
                    @foreach ($order->payments as $payment)
                        <tr class="text-center">
                            ...
                            <td class="py-2 border text-sm">
                                {{ $payment->paid_amount ? 'LKR ' . number_format($payment->paid_amount, 2) : '-' }}
                            </td>
                            <td class="py-2 border text-sm">
